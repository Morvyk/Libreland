...
impl LockSurface {
    ...
    pub(crate) fn pre_commit_hook<D: 'static>(_state: &mut D, _dh: &DisplayHandle, surface: &WlSurface) {
        ...
        compositor::with_states(surface, |states| {
            ...
            // Libreland patch: this hook outlives the role object. A locker
            // that destroys ext_session_lock_surface_v1 BEFORE its final
            // null-buffer unmap commit would otherwise hit the
            // CommitBeforeFirstAck error below (destroyed() reset
            // last_acked) and be disconnected mid-teardown. Same guard as
            // the wlr_layer patch.
            ...
            let has_buffer = match &surface_attrs.buffer {
                ...
                Some(BufferAssignment::Removed) => {
                    // Libreland patch: a NULL-buffer commit is how a client
                    // unmaps its lock surface during teardown
                    // (unlock_and_destroy): Qt/quickshell attach(null)+commit
                    // before destroying the objects. The spec calls this a
                    // protocol error, but treating it as fatal here
                    // *disconnects the locker mid-unlock* — which killed
                    // quickshell on every unlock and left the session with no
                    // bar. Tolerate it: ignore the unmap rather than kill the
                    // client. During an active lock this just means the
                    // surface stops updating (the lock stays up); it can
                    // never reveal the desktop.
                    return;
                }
