...
// Libreland vendor patch: HDR connector metadata applied via
// [`DrmSurface::set_hdr`].
...
impl DrmSurface {
    ...
    /// on the next modeset.
    ///
    /// **Libreland vendor patch** (not in upstream smithay 0.7). The HDR
    /// properties (`Colorspace`, `max bpc`, `HDR_OUTPUT_METADATA`) are
    /// folded into the surface's own atomic modeset commit — the only
    /// way to bring up an HDR link coherently (a separate side-channel
    /// commit wedges the pipe). Setting this dirties the pending state,
    /// so [`DrmSurface::commit_pending`] returns `true` and the next
    /// submit performs a modeset. HDR requires atomic KMS; on a legacy
    /// surface this returns [`Error::UnknownProperty`].
    pub fn set_hdr(&self, hdr: Option<atomic::HdrMetadata>) -> Result<(), Error> {
        match &*self.internal {
            DrmSurfaceInternal::Atomic(surf) => surf.set_hdr(hdr),
            DrmSurfaceInternal::Legacy(_) => Err(Error::UnknownProperty {
                handle: self.crtc.into(),
