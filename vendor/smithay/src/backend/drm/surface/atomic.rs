...
/// HDR connector properties to fold into an atomic modeset, set via
/// [`AtomicDrmSurface::set_hdr`].
///
/// **Libreland vendor patch** — upstream smithay 0.7 has no HDR support.
/// These properties (`Colorspace`, `max bpc`, `HDR_OUTPUT_METADATA`)
/// require a modeset and must ride in the *same* atomic commit that sets
/// the mode/plane, so they cannot be applied via a separate side-channel
/// commit. Resolving the concrete values (the `Colorspace` enum entry,
/// the `HDR_OUTPUT_METADATA` blob) is left to the caller, which has the
/// connector + fd needed to query/create them.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct HdrMetadata {
    /// Raw value for the connector's `Colorspace` enum (e.g. the entry
    /// named `BT2020_RGB` to enable, or `Default` to reset), or `None`
    /// to leave the property untouched.
    pub colorspace: Option<u64>,
    /// Desired `max bpc` (e.g. `10`), or `None` to leave it untouched.
    pub max_bpc: Option<u64>,
    /// Value for `HDR_OUTPUT_METADATA`: a blob handle obtained from
    /// `create_property_blob`, or `property::Value::Blob(0)` to clear it
    /// (return the link to SDR).
    pub metadata: property::Value<'static>,
}
...
pub struct State {
    ...
    /// Desired HDR connector properties (Libreland vendor patch). `None`
    /// leaves the connector in its default (SDR) signalling.
...
impl AtomicDrmSurface {
    ...
    /// Set (or clear with `None`) the HDR connector properties to apply
    /// on the next modeset.
    ///
    /// **Libreland vendor patch.** This only dirties the pending state;
    /// because [`State`] equality includes `hdr`, the change makes
    /// [`commit_pending`](Self::commit_pending) return `true`, so the
    /// next submit performs a real modeset (not a page flip) and folds
    /// the HDR properties into the *same* atomic commit that drives the
    /// pipe. After a successful commit the pending state is copied to
    /// current, so this self-clears (no perpetual modesets).
    pub fn set_hdr(&self, hdr: Option<HdrMetadata>) -> Result<(), Error> {
        let mut pending = self.pending.write().unwrap();
        pending.hdr = hdr;
        Ok(())
    }
...
impl<'a> AtomicRequest<'a> {
    ...
    // Libreland vendor patch: attach HDR connector properties. Each is
    // applied only if the connector actually exposes it (so a missing
    // `max bpc`/`Colorspace` on some drivers is silently skipped rather
    // than failing the whole modeset in `build()`).
    fn set_connector_hdr(&mut self, conn: connector::Handle, hdr: HdrMetadata) -> Result<(), Error> {
        let has_meta = self
            .mapping
            .conn_prop_handle(conn, "HDR_OUTPUT_METADATA")
            .is_ok();
        let has_colorspace = self.mapping.conn_prop_handle(conn, "Colorspace").is_ok();
        let has_max_bpc = self.mapping.conn_prop_handle(conn, "max bpc").is_ok();
        let connector_props = self.connector_props.entry(conn).or_default();
        if has_meta {
            connector_props.insert("HDR_OUTPUT_METADATA", hdr.metadata);
        }
        if let Some(colorspace) = hdr.colorspace {
            if has_colorspace {
                connector_props.insert("Colorspace", property::Value::UnsignedRange(colorspace));
            }
        }
        if let Some(max_bpc) = hdr.max_bpc {
            if has_max_bpc {
...
impl<'a> AtomicRequest<'a> {
    ...
    // Libreland vendor patch: attach HDR connector properties, each only
    // if the connector exposes it (so a missing property is skipped, not
    // a hard error).
    fn set_connector_hdr(&mut self, conn: connector::Handle, hdr: HdrMetadata) -> Result<(), Error> {
        if let Ok(handle) = self.mapping.conn_prop_handle(conn, "HDR_OUTPUT_METADATA") {
            self.request.add_property(conn, handle, hdr.metadata);
        }
        if let Some(colorspace) = hdr.colorspace {
            if let Ok(handle) = self.mapping.conn_prop_handle(conn, "Colorspace") {
                self.request
                    .add_property(conn, handle, property::Value::UnsignedRange(colorspace));
            }
        }
        if let Some(max_bpc) = hdr.max_bpc {
            if let Ok(handle) = self.mapping.conn_prop_handle(conn, "max bpc") {
                self.request
                    .add_property(conn, handle, property::Value::UnsignedRange(max_bpc));
            }
        }
        Ok(())
    }
...
impl<'a> AtomicRequest<'a> {
    fn build_request(
        ...
        // Libreland vendor patch: HDR connector properties folded into
        // this same atomic request (only meaningful on modeset paths;
        // page flips pass `None` since connector props persist).
        ...
        for conn in connectors {
            ...
            // Libreland vendor patch: attach HDR properties to the same
            // connector in the same modeset commit.
            if let Some(hdr) = hdr {
