...
| `window_minimize` | `220ms`, `ease-in`    | ✅    | A window minimizing: the same snapshot as a close, but shrinking harder and sinking as it fades, so being put away doesn't read as being destroyed. It sinks *downward* — a direction, not a destination, since the compositor has no idea where in a shell's bar a given window's entry sits. Restoring rises back the same distance, on **`window_open`**'s timing rather than this one: leaving wants to be brief and arriving wants to be seen, which is the split those two specs already encode. |
